...
function respond_ajax(){
	...
	$('#commentform').submit(function() {
		...
		if ( edit ) $('#comment').after('<input type="text" name="commentId" id="edit_id" value="' + edit + '" style="display:none;" />');
		/** Ajax */
		$.ajax({
			url: Always.ajaxurl+'comment/editComment',
			data: $(this).serialize(),
			type: $(this).attr('method'),
			error: function(data) {
				$('#loading').hide();
				$('#error').show().html(data.errorMsg);
				setTimeout(function() {$submit.attr('disabled', false).fadeTo('slow', 1); $('#error').hide();}, 5000);
			},
			success: function(data) {
				 if(data.success == false){
						...
						$('#new_comm_' + num).hide().append(data.successMsg);//插入新提交评论
						$('#new_comm_' + num + ' li').append(ok_htm);
						$('#new_comm_' + num).fadeIn("slow");//新提交成功评论渐现
...
function comment_respond(){	
	...
	if ( $( '.welcome' )[0] ){/////
		...
		$( 'span.info-edit' ).click(function(){
			$( 'span.info-edit' ).text().trim()=='修改' ? $( 'span.info-edit' ).html('取消修改') : $( 'span.info-edit' ).html('修改');
			$( '.author-info' ).toggle();
		});
	}
		...
		$( '.comment-list > li' ).each(function(){
			$(this).find('.avatar:first').hover(function(){
				...
				detail_time = setTimeout(function(){
					$.ajax({
						url:Always.ajaxurl+'comment/getCommentDetail',
						type:'POST',
						data:{ id: comment_id },
						beforeSend:function(){
							//左距离
